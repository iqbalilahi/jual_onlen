...
class Penjualan extends CI_Controller {
    ...
    public function tambah(){
        ...
        $this->data['all_barang'] = $this->products_model->lihat_stok();

        $this->load->view('penjualan/tambah', $this->data);
    }

    public function proses_tambah(){
        $jumlah_barang_dibeli = count($this->input->post('name_hidden'));
        ...
        for ($i=0; $i < $jumlah_barang_dibeli ; $i++) { 
            array_push($data_detail_penjualan, ['name' => $this->input->post('name_hidden')[$i]]);
            $data_detail_penjualan[$i]['no_penjualan'] = $this->input->post('no_penjualan');
            $data_detail_penjualan[$i]['price'] = $this->input->post('price_hidden')[$i];
        ...
        if($this->m_penjualan->tambah($data_penjualan) && $this->m_detail_penjualan->tambah($data_detail_penjualan)){
            for ($i=0; $i < $jumlah_barang_dibeli ; $i++) { 
                $this->products_model->min_stok($data_detail_penjualan[$i]['jumlah_barang'], $data_detail_penjualan[$i]['name']) or die('gagal min stok');
            }
    ...
    public function get_all_barang(){
        $data = $this->products_model->lihat_nama_barang($_POST['name']);
