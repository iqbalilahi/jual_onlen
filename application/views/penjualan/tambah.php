								...
								<form action="<?= base_url('penjualan/proses_tambah') ?>" id="form-tambah" method="POST">
									...
									<div class="form-row">
										...
										<div class="form-group col-3">
											...
											<input type="text" name="nama_admin" value="admin" readonly class="form-control">
										</div>
										<div class="form-group col-2">
										...
										<div class="form-group col-3">
											...
											<select name="name" id="name" class="form-control">
												...
												<?php foreach ($all_barang as $barang): ?>
													<option value="<?= $barang->name ?>"><?= $barang->name ?></option>
												<?php endforeach ?>
											</select>
										</div>
										<div class="form-group col-2">
											<label>ID</label>
											<input type="text" name="id" value="" readonly class="form-control">
										</div>
										<div class="form-group col-2">
											<label>Harga Barang</label>
											<input type="text" name="price" value="" readonly class="form-control">
										</div>
										<div class="form-group col-2">
											<label>Jumlah</label>
										...
										<table class="table table-bordered" id="keranjang">
											...
											<tfoot>
												<tr>
													...
													<td>
														<input type="text" name="total_hidden" readonly value="">
				...
				if($(this).val() == '') reset()
					...
					$.ajax({
						...
						success: function(data){
							...
							$('input[name="sub_total"]').val($('input[name="jumlah"]').val() * $('input[name="price"]').val())
			...
			$(document).on('click', '#tambah', function(e){
				...
				if(parseInt($('input[name="max_hidden"]').val()) <= parseInt(data_keranjang.jumlah)) {
					...
					$.ajax({
						...
						data: data_keranjang,
						success: function(data){
							if($('select[name="name"]').val() == data_keranjang.name) $('option[value="' + data_keranjang.name + '"]').hide()
							reset()
